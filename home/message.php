            ...
            <div class='jumbotron' id='curtain' style='background-image: url("../public/images/<?php echo $config[0]['curtain'] ?>")'>
                ...
                <div class="col-md-8">
                    <ul class="nav nav-tabs">
                        ...
                        <li class="active"><a href="message.php"> <span class="glyphicon glyphicon-equalizer "></span>站点留言</a></li>
                        <?php
                        if($state){
                            echo "
            ...
            <div class="well col-md-12">
                <div class="page-header">
                    ...
                    <form action="message.php" method="post">
                        <?php
                        if($state){
                            echo "<textarea type='text' name='text' style='height:250px;width:100%;font-size:25px;' class='col-md-12'></textarea>";
                            echo "<h3><button type='submit' name='btnMessage' class='btn btn-info btn-lg'>发表</button></h3>";
                        }else{
                    ...
                    if(isset($_POST['btnMessage']) && $_POST['text'] != null){
                        $text = $_POST['text'];
                        $username = $_SESSION['username'];
                        $userArray = $pdo->select("select * from users where username = '$username'");
                        $userId = $userArray[0]['id'];
                        $messageSql = "insert into message(content,time,user_id,ip) 
                                      values('$text','$showtime','$userId','$ip')";
                        $test = $pdo->exec($messageSql);
                ...
                <!----------------------------------------------->
                <?php
                $paginationArray = $pdo->friendPag('message','id',@$text,10);
                ...
                $messageSql = "select m.id,m.content,m.time,u.username,m.ip from  message m inner join users u on m.user_id = u.id order by time desc limit $offset,$length";
                ...
                <?php foreach ($messageArray as $value){?>
                    <div class="well">
                        ...
                        <a href="#" style="font-size:13px;padding-top:2px;"> <span class="glyphicon glyphicon-tags"></span> 留言IP:<?php echo $value['ip']; ?></a>
                        <p>
